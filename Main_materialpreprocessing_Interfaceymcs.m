function [] = Main_materialpreprocessing_Interfaceymcs(InputFile, xyInterface1,xyInterface2,NumInterface)
...
for iele = 1 : NumElem 
    ...
    if yMax <= yInter(1) + GeoTol
        ME.mat = 1;
    elseif yMin >= yInter(1) - GeoTol && yMax <= yInter(2) + GeoTol
        ME.mat = 2;
    else
         ME.mat = 3;
    end
%     [ino, ono] = inpolygon(xy_poly(:,1),xy_poly(:,2),xyInterface11(:,1),xyInterface11(:,2));
%     [int, ont] = inpolygon(xy_poly(:,1),xy_poly(:,2),xyInterface21(:,1),xyInterface21(:,2));
% %     [ins, ons] = inpolygon(xy_poly(:,1),xy_poly(:,2),xyInterface31(:,1),xyInterface31(:,2));
%     if sum(ino)==size(xy_poly,1)
%         ME.mat = 1;
%     elseif sum(int)==size(xy_poly,1)
%         ME.mat = 3; 
%     else
%         ME.mat = 2; 
%     end
...
% material interface 
[nInterface, Interfaces] = FormMulInterfaceUnitSquareymcs(xyInterface1,xyInterface2,NumInterface);
