...
function [Kuu, Kpp, Mpp, Kup, Spp]=CoeffMatBiot2D_Consolidation(DMatU, DMatP, ....
    M, alpha, tao)
...
elseif strcmpi(MeshShape,'BiotQ4Q4') || strcmpi(MeshShape,'Q4') || ... 
    ...
    ngauss = 4; % number of gauss points in 1 direction 
...
for iele = 1 : NumElem
    ...
    SePP = zeros(length(DOFs_P), length(DOFs_P));   % S in Wentao Li's paper
    INp = zeros(1, length(DOFs_P));   % integration of shape function matrix for fluid
    ...
    for iquad=1:NumIntegQuad     % loop of quadrilaterals
        ...
        for igauss=1:length(wgt)    % loop of integration points
            ...
            SePP = SePP + tao * (Np.' * Np) * detJ_i * wgt(igauss);
            INp = INp + Np * detJ_i * wgt(igauss);
...
%     MePP_s = MePP * beta;    
    ...
    Spp(DOFs_P, DOFs_P) = Spp(DOFs_P, DOFs_P) - tao / Ae * (INp.' * INp);
